...
namespace Syft {
    ...
    BestEffortSynthesisResult FONDBestEffortReachabilitySynthesizer::run() {
        ...
        // does not correctly evaluate env_error var
        CUDD::BDD adv_final_states = (env_error_var + domain_dfa.final_states());
        // CUDD::BDD adv_final_states = domain_dfa.final_states();
        // domain_dfa.final_states();
        ...
        // TODO. Implement invariant automated construction
        // currently invariant has only agent error var
        CUDD::BDD invariant_bdd = domain.get_invariants_bdd();

        // synthesize strategy
        // TODO. Synthesis process requires too much time 
        // Can we use invariants as in planning to prune the state space?
        ...
        CoOperativeReachabilitySynthesizer coop_synthesizer(
            ...
            Player::Agent,
            Player::Agent,
            coop_final_states * invariant_bdd, 
            invariant_bdd
        );
    ...
    void FONDBestEffortReachabilitySynthesizer::interactive(const Domain& domain, const SymbolicStateDfa& domain_dfa, const BestEffortSynthesisResult& result) const {
        ...
        CUDD::BDD agent_error_var = var_mgr_->state_variable(domain_dfa.automaton_id(), domain_vars.size()+1);
        CUDD::BDD env_error_var = var_mgr_->state_variable(domain_dfa.automaton_id(), domain_vars.size());
        ...
        // order of variables (F, EnvErr, Act, React, AgErr)
        ...
        while (true) {
            ...
            std::vector<int> state_prime(state.begin(), state.begin() + state.size() - 1); // F, EnvErr
            for (int i = 0; i < var_mgr_->output_variable_count(); ++i) state_prime.push_back(0); // Act
            for (int i = 0; i < var_mgr_->input_variable_count(); ++i) state_prime.push_back(0); // React
            state_prime.push_back(state[state.size() - 1]); // AgErr
            ...
            if (final_states.Eval(state_prime.data()).IsOne())
                {std::cout << "FINAL. The agent reached the goal. Termination" << std::endl; return;}
            else {std::cout << "NOT FINAL" << std::endl;}
            std::cout << std::endl;

            std::vector<int> transition(state.begin(), state.begin() + state.size() - 1);
            
            if (win_region.Eval(state_prime.data()).IsOne()) {
            ...
            } else if (coop_region.Eval(state_prime.data()).IsOne()) {
            ...
            for (int i = domain_dfa_vars - 1; i < domain_dfa_vars + var_mgr_->output_variable_count(); ++i) {
                ...
                if (var_mgr_->is_output_variable(var)) {
                    ...
                    agent_eval = output_function[i].Eval(state_prime.data()).IsOne();
