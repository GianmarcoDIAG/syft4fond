...
namespace Syft {
    ...
    SymbolicStateDfa Domain::to_symbolic() {
        ...
        invariants_bdd_ *= agent_env_mutex_axioms.first;
        // std::cout << "Done!" << std::endl;
    ...
    std::vector<CUDD::BDD> Domain::get_transition_function(std::size_t automaton_id, const CUDD::BDD& agent_mutex, const CUDD::BDD& env_mutex) const {
        ...
        for (const auto& act : actions_) {
            ...
            for (const auto& i : act_pos_pre) act_pre_bdd = act_pre_bdd * var_mgr_->state_variable(automaton_id, i);
            for (const auto& i : act_neg_pre) act_pre_bdd = act_pre_bdd * (!var_mgr_->state_variable(automaton_id, -i));
            ...
            for (const auto& id : act_add_list) {
                ...
                add_bdds[id] = add_bdds[id] + (act.get_action_bdd() * act_pre_bdd);
            }
            for (const auto& id : act_del_list) {
                // debug
                // std::cout << "Action: " << act.get_action_name() << ". Added to var " << id << " BDD del list" << std::endl;
                del_bdds[id] = del_bdds[id] + (act.get_action_bdd() * act_pre_bdd);
            }
        }
        // std::cout << "DONE!" << std::endl;
